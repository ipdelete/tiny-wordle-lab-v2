...
The primary metric is **fixed-state usable-policy rate** on the 47 held-out post-opening states. Count a response as successful only when it returns a format-valid five-letter guess, avoids every guess in the supplied history, and satisfies all prior feedback. Both models receive the same states, so model behavior cannot change the denominator or difficulty mix. Evaluate each state under the training prompt and deployment prompt; the training-format result carries the primary data comparison.
